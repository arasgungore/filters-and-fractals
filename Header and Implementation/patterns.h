...
static PPMImage* printRGBstrips(unsigned int strip_width, unsigned int strip_height, unsigned int iteration) {
	PPMImage *img = createPPMImage(strip_width,3*strip_height*iteration,BLACK);
...
static PPMImage* printRainbow(unsigned int strip_width, unsigned int strip_height, unsigned int iteration) {
	PPMImage *img = createPPMImage(strip_width,7*strip_height*iteration,BLACK);
	unsigned int i,j,k,counter=0;
	for(k=0;k<iteration;k++) {
		for(i=0;i<strip_height;i++)
			for(j=0;j<strip_width;j++)
...
void drawCircle(int xc, int yc, int x, int y, PPMImage *img, PPMPixel color) {
	...
	img->data[center_index + xc + x - (yc + y)*img->width - 1] =
	img->data[center_index + xc - x - (yc + y)*img->width - 1] =
	img->data[center_index + xc + x - (yc - y)*img->width - 1] =
	img->data[center_index + xc - x - (yc - y)*img->width - 1] =
	img->data[center_index + xc + y - (yc + x)*img->width - 1] =
	img->data[center_index + xc - y - (yc + x)*img->width - 1] =
	img->data[center_index + xc + y - (yc - x)*img->width - 1] =
	img->data[center_index + xc - y - (yc - x)*img->width - 1] = color;
}
...
void drawLine(int x1, int y1, int x2, int y2, PPMImage *img, PPMPixel color) {
	...
	for(i=0; i<=steps; i++, x+=Xinc, y+=Yinc)
		img->data[center_index + (int)x - img->width*(int)y - 1] = color;
}
